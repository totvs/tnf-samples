...
public class CreatePurchaseCommandHandler : CommandHandler<CreatePurchaseCommand, CreatePurchaseResult>
{
    ...
    public override async Task<CreatePurchaseResult> ExecuteAsync(CreatePurchaseCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var newPurchase = new Domain.Entities.Purchase(command.CarId, command.CustomerId, command.Price, command.PurchaseDate, command.StoreId);

        newPurchase = await _purchaseRepository.InsertAsync(newPurchase, cancellationToken);

        _logger.EntitySuccessfullyCreated("Purchase", newPurchase.Id);

        return new CreatePurchaseResult(newPurchase.Id);
    }    
}
