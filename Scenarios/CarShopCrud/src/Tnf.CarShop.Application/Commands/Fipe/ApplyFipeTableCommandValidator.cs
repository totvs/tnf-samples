    ...
    public override void Configure()
    {
        RuleFor(command => command.FipeCode)
            ...
            .WithTnfNotification(LocalizationSource.Default, LocalizationKeys.PropertyRequired,
                nameof(ApplyFipeTableCommand.FipeCode));

        RuleFor(command => command.MonthYearReference)
            .NotEmpty()
            .WithTnfNotification(LocalizationSource.Default, LocalizationKeys.PropertyRequired,
                nameof(ApplyFipeTableCommand.MonthYearReference));

        RuleFor(command => command.Brand)
            .NotEmpty()
            .WithTnfNotification(LocalizationSource.Default, LocalizationKeys.PropertyRequired,
                nameof(ApplyFipeTableCommand.Brand));
        ...
        RuleFor(command => command.Model)
            .NotEmpty()
            .WithTnfNotification(LocalizationSource.Default, LocalizationKeys.PropertyRequired,
                nameof(ApplyFipeTableCommand.Model));
