...
namespace Tnf.Sgdp.GuineaPig.Controllers
{
    ...
    public class CustomerController : TnfController
    {
        ...
        /// <summary>
        /// Gets the customer's national identification data 
        /// </summary>
        /// <param name="id">Customer id</param>
        /// <returns>Customer identification data</returns>
        [HttpGet("{id}/nationalid")]
        public async Task<IActionResult> GetNationalIdentification(Guid id)
        {
            var customerData = await _appService.GetIdentificationsAsync(id);

            return CreateResponseOnGet(customerData, _name);
        }
